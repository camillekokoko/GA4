...
AND '20210904'
AND event_name IN ( 'add_to_cart', 'session_start'))

SELECT 
isoweek,
device_platform,
SAFE_DIVIDE(addtcart_event_count, session_start_event_count)as CVR FROM ( 
SELECT
...
COUNT(DISTINCT CASE WHEN event_name = 'add_to_cart' THEN  CONCAT(user_pseudo_id, "-", session_id) END) AS addtcart_session_count,
COUNT(DISTINCT CASE WHEN event_name = 'add_to_cart' THEN  CONCAT(user_pseudo_id, "-", event_timestamp)END) AS addtcart_event_count,
COUNT(DISTINCT CASE WHEN event_name = 'add_to_cart' THEN user_pseudo_id END) AS addtcartuser_count
FROM
pages
GROUP BY
1,2
