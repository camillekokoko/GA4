...
WHERE
_table_suffix BETWEEN '20210904'
AND '20210904'
AND event_name IN ( 'view_item', 'session_start') AND platform = 'WEB' 
...
date,
device_platform,
SAFE_DIVIDE(viewitem_session_count, session_start_session_count) as session,
SAFE_DIVIDE(viewitem_event_count, session_start_event_count) as event,
SAFE_DIVIDE(viewitem_user_count, session_start_user_count)as user FROM (
SELECT
date,
device_platform, 
#country,
...
COUNT(DISTINCT CASE WHEN event_name = 'view_item' THEN  CONCAT(user_pseudo_id, "-", session_id) END) AS viewitem_session_count,
COUNT(DISTINCT CASE WHEN event_name = 'view_item' THEN  CONCAT(user_pseudo_id, "-", event_timestamp)END) AS viewitem_event_count,
COUNT(DISTINCT CASE WHEN event_name = 'view_item' THEN user_pseudo_id END) AS viewitem_user_count
FROM
pages
GROUP BY
1,2
